   ...
   const APiHAbitaciones=async()=>{
      try{
         ...
         if(response.status===200){
          ...
          const filter=data.filter((item)=>item.opciones==="Suite Estandar")
          if(filter){
            setBAsico(filter)   
  ...
  <div   key={item._id}
    ...
    transition={{ duration: 0.5, delay: 0.1 }} className='div-cuartos'>
      ...
      <div className='info'>
         ...
         {
         logeado.length>0?<button  className='reservar'>RESERVAR AHORA</button>:<button className='reservar'   data-bs-toggle="modal" data-bs-target="#exampleModal"
